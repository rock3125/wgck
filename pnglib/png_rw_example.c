...
int CheckPNG( const char *file_name )
{
	...
	ret = png_check_sig((png_bytep)buf, 8);
    fclose(fh);
	return ret;
}
...
static void user_read_fn( png_structp png_ptr, png_bytep data, png_size_t length )
{
	...
	check = (png_size_t)fread( n_data, (png_size_t)1, length, io_ptr );
	if ( check!=length )
	{
...
*
*/
int ReadPNG( const char* fileName, char* errStr )
{
	png_structp		png_ptr;
	png_infop		info_ptr;
	FILE*			fh;
	...
	png_bytep*		row_pointers = NULL; /* for loading rows of the image */
