...
char			versionString[] = "Excession Engine v 0.79 alpha";
			   		  ...
			   		  int nCmdShow )
{
	...
	// doh!
    InitialiseStartup( fname, texturePath, hInst, app, 
					   appName, appTitle, fog, errStr );
    PostCond( app!=NULL );
    if ( errStr.length()>0 )
    {
...
bool SetupWindow( void )
{
	...
	char tempError[256];
	if ( !aboutJPEG.Load(hInstance,MAKEINTRESOURCE(IDR_SPLASHSCREEN),tempError) )
	{
		WriteString( tempError );
		return false;
	}
...
void DoPaint( HWND hWnd )
{
	if ( active )
	{
		...
		{
			startTimerTime = ::GetTickCount();
			float numSeconds = float(deltat) / 1000.0f;

			// after 10 seconds - start adaptive frame measuring
			fps = float(frameCounter) / numSeconds;
			frameCounter = 0;
		}
...
{
	if ( firstTime && initialised )
	{
		firstTime = false;
		if ( fullScreen && !mouseCaptured )
		{
			::SetCapture(hWnd);
			::SetCursor( NULL );
			::SetMenu(hWnd,NULL);
...
bool GetRegistrySettings( void )
{
	...
	if ( GetRegistryKey( "Software\\PDV\\Performance", "Fog", fs ) )
		if ( fs )
			fog = true;
		else
			fog = false;
