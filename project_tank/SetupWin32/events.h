...
public:
    ...
    // Event system
    virtual bool	Setup( HWND _hwnd, float depth, int widht, int height, char* errStr );
	virtual bool	StartupSystem( HWND hwnd, char* errStr );
	...
	int		SetDCPixelFormat( HDC hDC, int bitDepth, char* errStr );
...
private:
	...
	bool DXSetup( HWND hWnd, bool fullScreen, int width, int height, int depth, char* errStr );
	void DXError( int hRes, const char* routine, char* errStr );
