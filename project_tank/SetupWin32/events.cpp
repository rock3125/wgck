...
//
bool TEvent::Setup( HWND _hwnd, float depth, int width, int height, char* errStr )
{
	if ( !initialised )
    {
    	strcpy( errStr, "Initialisation of setupGL.dll failed" );
        return false;
    }
	Window( _hwnd );
...
//	Callback event - can be overwritten by user through inheritance
//
bool TEvent::StartupSystem( HWND, char* )
{
	return true;
};
...
//
int TEvent::SetDCPixelFormat( HDC hDC, int bitDepth, char* errStr )
{
	...
	if ( pf==0 )
	{
	   	sprintf( errStr, "Can't find an accelerated openGL driver for this system" );
		return 0;
	}
	char* buf = new char[256];
	sprintf(buf,"Selected pixel format %d", pf );
	...
	{
		sprintf( errStr, "TEvent::SetPixelFormat failed (%ld)", GetLastError() );
		return 0;
	}
...
bool TEvent::DXSetup( HWND hWnd, bool fullScreen, int width, int height, int bdepth, char* errStr )
{
	errStr = NULL;
	if (!fullScreen)
	{
		...
		if ( hRes!=DD_OK )
		{ 
			strcpy(errStr, "Error: SetCooperativeLevel failed" );
			return false; 
		}
		...
		if ( hRes!=DD_OK ) 
		{ 
			strcpy(errStr,"Error: CreateClipper failed" );
			return false; 
		}
		...
		if ( hRes!=DD_OK ) 
		{ 
			strcpy(errStr, "Error: CreateSurface failed" );
			return false; 
		}
		...
		if ( hRes!=DD_OK ) 
		{ 
			strcpy(errStr,"Error: Create back surface failed");
			return false; 
		}
	}
	else
	{
		...
		if ( hRes!=DD_OK ) 
		{ 
			strcpy(errStr, "Error: could not set the required resolution" );
			return false; 
		}
...
void TEvent::DXError( int hRes, const char* routine, char* errStr )
{
	strcpy( errStr, "\n************\nError: " );
	strcat( errStr, routine );
	strcat( errStr, ": " );
	switch ( hRes )
	{
		case DDERR_INCOMPATIBLEPRIMARY:
			{
				strcat( errStr, "DDERR_INCOMPATIBLEPRIMARY" );
			}
			break;

		case DDERR_INVALIDCAPS:
			{
				strcat( errStr, "DDERR_INVALIDCAPS" );
			}
			break;

		case DDERR_INVALIDOBJECT:
			{
				strcat( errStr, "DDERR_INVALIDOBJECT" );
			}
			break;

		case DDERR_INVALIDPARAMS:
			{
				strcat( errStr, "DDERR_INVALIDPARAMS" );
			}
			break;

		case DDERR_INVALIDPIXELFORMAT:
			{
				strcat( errStr, "DDERR_INVALIDPIXELFORMAT" );
			}
			break;

		case DDERR_NOALPHAHW:
			{
				strcat( errStr, "DDERR_NOALPHAHW" );
			}
			break;

		case DDERR_NOCOOPERATIVELEVELSET:
			{
				strcat( errStr, "DDERR_NOCOOPERATIVELEVELSET" );
			}
			break;

		case DDERR_NODIRECTDRAWHW:
			{
				strcat( errStr, "DDERR_NODIRECTDRAWHW" );
			}
			break;

		case DDERR_NOEMULATION:
			{
				strcat( errStr, "DDERR_NOEMULATION" );
			}
			break;

		case DDERR_NOEXCLUSIVEMODE:
			{
				strcat( errStr, "DDERR_NOEXCLUSIVEMODE" );
			}
			break;

		case DDERR_NOFLIPHW:
			{
				strcat( errStr, "DDERR_NOFLIPHW" );
			}
			break;

		case DDERR_NOMIPMAPHW:
			{
				strcat( errStr, "DDERR_NOMIPMAPHW" );
			}
			break;

		case DDERR_NOOVERLAYHW:
			{
				strcat( errStr, "DDERR_NOOVERLAYHW" );
			}
			break;

		case DDERR_NOZBUFFERHW:
			{
				strcat( errStr, "DDERR_NOZBUFFERHW" );
			}
			break;

		case DDERR_OUTOFMEMORY:
			{
				strcat( errStr, "DDERR_OUTOFMEMORY" );
			}
			break;

		case DDERR_OUTOFVIDEOMEMORY:
			{
				strcat( errStr, "DDERR_OUTOFVIDEOMEMORY" );
			}
			break;

		case DDERR_PRIMARYSURFACEALREADYEXISTS:
			{
				strcat( errStr, "DDERR_PRIMARYSURFACEALREADYEXISTS" );
			}
			break;

		case DDERR_UNSUPPORTEDMODE:
			{
				strcat( errStr, "DDERR_UNSUPPORTEDMODE" );
			}
			break;

		default:
			{
				strcat( errStr, "unknown" );
			}
			break;
	}
	strcat( errStr, "\n" );
...
bool TEvent::SetupDirectX( HWND hWindow, bool fullScreen, int Wxsize, int Wysize, 
						   int Wbitdepth, char* errStr )
{
	int hRes=DirectDrawCreateEx(NULL,(LPVOID*)&lpDD,IID_IDirectDraw7,NULL);
	if ( hRes!=DD_OK) 
	{ 
		strcpy( errStr, "Error: Could not create DirectX7 object" );
		return false; 
	}
