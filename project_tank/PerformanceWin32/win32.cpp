...
#include <PerformanceWin32/win32Resource.rh>
...
char			versionString[] = "Performance Test v 1.01";
			   		  ...
			   		  int nCmdShow )
{
	...
	strcat( logfile, "performancelog.txt" );
	SetLogFile( logfile );
#endif

//	ParseCommandLine( lpCmdLine, fname, texturePath );

	// exit if incorrect registry settings are found
...
//	mainMenu = ::LoadMenu( hInstance, MAKEINTRESOURCE(IDR_MENU1) );
    ...
    hWindow = hwnd;
	if ( !app->SetupDirectX(hWindow,fullScreen,Wxsize,Wysize,
							Wbitdepth,errStr) )
	{
		WriteString( errStr.c_str() );
		return false;
    ...
    //
    float fps1 = 1000.0f;
    fps1 = fps1 / float(kFPS);
    if ( SetTimer( hwnd,1,int(fps1),NULL)==0 )
	{
		app->DXCleanup(hWindow);
	...
	ConsolePrint( "-15				 15 bit colour" );
	ConsolePrint( "-16				 16 bit colour" );
	ConsolePrint( "-24				 24 bit colour" );
	ConsolePrint( "-32				 32 bit colour" );
...
void DoPaint( HWND hWnd )
{
	if ( active )
	{
		...
		{
			char buf[256];
			fps = float(fixedFrameCounter) / float(numSeconds);
...
//			::TextOut( hdc, 10,10, buf, strlen(buf) );
			...
			if ( msgStr.length()>0 )
				::TextOut( hdc, 80,10, msgStr.c_str(), msgStr.length() );
		}
	}
	else
	{
...
void DoTimer( HWND hWnd, WPARAM wParam )
{
	...
	if ( app!=NULL )
	{
		...
		if ( app->Finished() )
		{
			...
			HKEY  keyRef;
			DWORD result;
			LONG res = RegCreateKeyEx( HKEY_LOCAL_MACHINE, "Software\\PDV\\Performance",
									 0, NULL, REG_OPTION_NON_VOLATILE, 
									 KEY_SET_VALUE, NULL, &keyRef, &result );
			if ( res==ERROR_SUCCESS )
			{
				res = RegSetValueEx( keyRef, "TestCompleted", 
									 0, REG_DWORD, (const BYTE*)&stfps, 
									 sizeof(size_t) );
				if ( res==ERROR_SUCCESS )
				{
					RegCloseKey( keyRef );
...
void ParseCommandLine( const TString& lpCmdLine, TString& fname, TString& texturePath )
{
	TString switches;
	...
	if ( lpCmdLine[0]!='-' )
	{
		...
		if ( lpCmdLine[i]==' ' )
		{
			fname = const_cast<TString&>(lpCmdLine).substr(0,i);
			texturePath = &lpCmdLine[i+1];
...
bool GetRegistrySettings( void )
{
	...
	if ( GetRegistryKey( "Software\\PDV\\Performance", "Resolution", res ) )
	{
		switch ( res )
		{
			...
			default:
				{
					MessageBox( NULL, "Unknown resolution setting in registry", "Error",
								MB_OK|MB_ICONERROR );
					return false;
				}
		}
	...
	size_t bits;
	if ( GetRegistryKey( "Software\\PDV\\Performance", "BitDepth", bits ) && fullScreen)
	{
		switch ( bits )
		{
			case 0:
			...
			default:
				{
					MessageBox( NULL, "Unknown bitdepth setting in registry", "Error",
								MB_OK|MB_ICONERROR );
					return false;
				}
		}
