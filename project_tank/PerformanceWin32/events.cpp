...
bool TEvent::DXSetup( HWND hWnd, bool fullScreen, int width, 
					  int height, int bdepth, TString& errStr )
{
	errStr = "";
	if (!fullScreen)
	{
	...
	else
	{
		...
		// default bit depth just in case non selected
		if ( bdepth==0 )
			bdepth = 16;
...
size_t TEvent::PolygonsPerSecond( float fps ) const
{
