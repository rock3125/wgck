...
// these were put in here to get wrid of some nasty warning
// messages
...
typedef struct {
  ...
  /* buffer interface */
  JOCTET * inptr;				/* input buffer position - current location */
  JOCTET * endptr;				/* input buffer position - end byte */
...
buffer_fill_input_buffer (j_decompress_ptr cinfo)
{
	...
	if ( ((src->endptr+1) - src->inptr) > INPUT_BUF_SIZE )
		nbytes = INPUT_BUF_SIZE;
	else
		nbytes = (src->endptr+1) - src->inptr;

	if (nbytes > 0)
	{
		// #define MEMCOPY(dest,src,size)
		MEMCOPY( src->buffer, src->inptr, nbytes);
		src->inptr += nbytes;
	}
 ...
 * The caller must have already opened the data, and is responsible
 * for killing it after finishing decompression.
 */
